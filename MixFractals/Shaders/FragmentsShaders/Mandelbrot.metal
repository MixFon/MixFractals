...
fragment float4 fragment_mandelbrot_fractal(VertexOut in [[stage_in]], constant FractalUniforms& uniforms [[buffer(0)]]) {
	// Переводим экранные координаты (uv) в комплексную плоскость
	// Вычитаем 0.5 чтобы центр прямоугольника (экрана был в 0,0)
	float2 c = float2(
		(in.uv.x - 0.5) * uniforms.scale * uniforms.aspect + uniforms.point.x,
		(in.uv.y - 0.5) * uniforms.scale + uniforms.point.y
	);
	...
	while (dot(z, z) < 2.0 && iter < uniforms.maxIter) {
		z = complexPow(z, uniforms.power) + c;
		iter++;
	}
